...
import { GameState, Question as QuestionType, FeedbackItem, QuestionData, APIResponse } from './types';

const TOTAL_TIME = 30;


// Transform raw question data into a usable format for the app

function transformQuestion(q: QuestionData): QuestionType {
  return {
    id: q.questionId,
    sentence: q.question,
    words: q.options,
    blanks: q.question.split('_____________').length - 1,
...
function App() {
  ...
  const [gameState, setGameState] = useState<GameState>({
    currentQuestion: -1, // -1 means game hasn't started
    score: 0,
    answers: {},
    timeRemaining: TOTAL_TIME,
  });
  const [feedback, setFeedback] = useState<FeedbackItem[]>([]);
  const [userAnswers, setUserAnswers] = useState<string[][]>([]); // Add this line

   // Load mock questions on component mount
  ...
  useEffect(() => {
    ...
    const transformedQuestions = mockData.data.questions.map(transformQuestion);
  ...
  useEffect(() => {
    let timer: number;
    if (gameState.currentQuestion >= 0 && gameState.timeRemaining > 0) {
  ...
  }, [gameState.currentQuestion, gameState.timeRemaining]);
  ...
  const handleStart = () => {
    ...
    // Triggered when time runs out for a question
  const handleTimeUp = () => {
    const currentQuestion = questions[gameState.currentQuestion];
    const emptyAnswer = Array(currentQuestion.blanks).fill('');
    handleNextQuestion(emptyAnswer);
  };
  ...
  const handleNextQuestion = (answer: string[]) => {
    ...
    newUserAnswers[gameState.currentQuestion] = answer;
    setUserAnswers(newUserAnswers);

    if (gameState.currentQuestion === questions.length - 1) {
      // Game is complete, generate feedback for all questions
      const allFeedback = questions.map((q, index) => ({
        question: q.sentence,
        userAnswer: newAnswers[index] || Array(q.blanks).fill(''),
        correctAnswer: q.correctAnswer,
        isCorrect: JSON.stringify(newAnswers[index]) === JSON.stringify(q.correctAnswer),
      }));
  ...
  if (gameState.currentQuestion >= questions.length) {
    return (
      ...
      correctAnswers={questions.map(q => q.correctAnswer)} // Now passing string[][]
  ...
  return (
    ...
    correctAnswers={questions[gameState.currentQuestion].correctAnswer} // Add this line
